    ...
    <div class="footer">
      <div id="controls" class="btn-group">
        ...
        <button
          ...
          @click="$emit('click:auto-solve')"
        >Auto Solve</button>
        <button id="pause" class="btn btn-default" @click="$emit('click:pause')">Pause</button>
        <button
          id="print-positions"
          class="btn btn-default"
          @click="$emit('click:print-positions')"
        >Print Positions</button>
      </div>
...
export default class SearchVisualizer extends Vue {
  ...
  chooseClass(sub: string) {
    ...
    if (this.output) {
      solution =
        sub.includes("Solution found") || sub.includes("New best path");
