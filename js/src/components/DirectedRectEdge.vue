<script lang="ts">
  ...
  export default class DirectedRectEdge extends BaseEdge {
    ...
    /** The x-coordinate and y-coordinate to place the start of the path. It is adjusted to be on the edge of the node. */
    adjustedStart() {

    }

    /** The x-coordinate and y-coordinate to place the end of the path. It is adjusted to be on the edge of the node. */
    adjustedEnd() {
      // First make four linear test lines (y = mx + b) along the four boundaries of the target box.
      const xLeft = this.x2 - this.graph_node_width/2;
      const xRight = this.x2 + this.graph_node_width/2;
      ...
      // setting the cut lines for x and y 
      const xBound = this.x1 < this.x2 ? xLeft : xRight;
      const yBound = this.y1 < this.y2 ? yUp : yDown;


      const cutByXBoundPoint = this.intersectX(xBound);
      const cutByYBoundPoint = this.intersectY(yBound);


      const cutByXBoundNewY = cutByXBoundPoint.y;
      const cutByYBoundNewX = cutByYBoundPoint.x;


      //return the pair of x-coordinate and y-coordinate that touches the node
      if(cutByXBoundNewY<yUp || cutByXBoundNewY>yDown){
        return cutByYBoundPoint;
      } else {
        return cutByXBoundPoint;
      }
